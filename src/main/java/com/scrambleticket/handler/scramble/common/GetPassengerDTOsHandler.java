...
public class GetPassengerDTOsHandler implements FlowHandler {
    ...
    public void handle(FlowContext context, FlowHandlerChain chain) {
        ...
        String repeat_submit_token = trainScrambleContext.getREPEAT_SUBMIT_TOKEN(); // candidate情况无token

        ByteBuf body = StringUtil.isBlank(repeat_submit_token) ? Unpooled.EMPTY_BUFFER :
