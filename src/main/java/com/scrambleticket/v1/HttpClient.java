...
public class HttpClient implements ApplicationContextAware {
    ...
    public static <T> ResponseEntity<T> get(String url, String header, Class<T> type) {
        ...
        // get 添加 header
        RequestCallback requestCallback = HttpClient.getRestTemplate().httpEntityCallback(entity, type);
        ResponseExtractor<ResponseEntity<T>> responseExtractor =
            HttpClient.getRestTemplate().responseEntityExtractor(type);
        return HttpClient.getRestTemplate().execute(url, HttpMethod.GET, requestCallback, responseExtractor);
    }
    ...
    public static <T> ResponseEntity<T> post(String url, String header, Object body, Class<T> type) {
        ...
        return HttpClient.getRestTemplate().postForEntity(url, entity, type);
    }
    ...
    private static CloseableHttpClient createClient() {
        ...
        RequestConfig requestConfig =
            RequestConfig.custom().setSocketTimeout(60000).setConnectTimeout(60000).setConnectionRequestTimeout(10000)
                // TODO 非重定向
                .setRedirectsEnabled(false).build();

        return HttpClients.custom().setDefaultRequestConfig(requestConfig).setConnectionManager(connectionManager)
            .evictExpiredConnections().evictIdleConnections(30, TimeUnit.SECONDS)
            .setProxy(new HttpHost("localhost", 8888, "http")).build();
    }
